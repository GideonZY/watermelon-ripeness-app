...
这是一个教学型、可运行的 MVP：录制 2.5 秒单声道 16 kHz PCM，保存 WAV，计算主频、频谱质心、RMS 能量和前后段衰减，再用透明的占位规则输出“偏生 / 适熟 / 偏熟”。**当前判断规则没有经过真实数据验证，不能用于商业分级或食品质量保证。**
...
WAV 保存在应用专属 Music 目录；界面会显示完整路径。卸载 App 会删除这些录音，采集正式数据时请及时复制到电脑。

## 使用方法

- 环境尽量安静；手机麦克风距瓜表约 10 cm，位置和角度保持一致。
- 点击“开始检测”后，在 2.5 秒内用同一根手指、相近力度拍 2～3 次。
- 每个瓜建议在不同位置录 3 条；不要拿同一次录音重复充数。
...
不同品种、产地和季节的阈值可能不同，务必记录这些字段。CSV 模板位于 `data/samples_template.csv`，每行对应一条 WAV；`sample_id` 应表示“瓜 ID”，同一瓜的三条录音使用相同 ID 或增加独立 `recording_id` 字段。
