...
这是一个教学型、可运行的 MVP：录制 2.5 秒单声道 16 kHz PCM，在内存中计算主频、频谱质心、RMS 能量和前后段衰减，再用透明的占位规则输出“偏生 / 适熟 / 偏熟”。v0.2.0 增加了“生—熟—过熟”指针仪表盘并取消检测录音落盘；v0.2.2 在此基础上增加 100 ms 实时频率刷新。**当前判断规则没有经过真实数据验证，不能用于商业分级或食品质量保证。**
